...
1. Find the count of customers from each city.
2. Find the count of customers per city per street name.
3. Find the total amount in stock of each item type.

## HAVING Clause

4. Find the count of customers per city per street name with counts greater than 5.
5. Find the total stock for item types that have a total stock greater than 700.

## IF Statements

6. Label each item's stock level: "High stock" if amount_in_stock >= 30, otherwise "Low stock".
7. Add a column `High_stock` that defaults to True if stock >= 30, otherwise False.

## CASE Statements

8. Label each item's stock level:
   - Low stock if amount_in_stock < 25
   - Medium stock if between 25 and 50
   - High stock if greater than 50

9. Categorize customers by age:
   - Teen: below 20 years
   - Young adult: 20–30 years
   - Adult: 31–40 years
   - Middle-aged: over 40

## CASE with GROUP BY & Aggregation

10. Find the number of customers in each age category.
11. Find the number of items in each stock level category.
12. Label the average price of each item type as:
    - Low: avg < 12
    - Medium: 12–32
    - High: > 32
...
13. Add a column `age_category` and fill it with values based on age brackets.

## CASE inside Aggregate Functions

14. Count the number of customers in each age category per city.

## JOINS

15. Use INNER JOIN to find customers along with their order information.
16. Show customer name, city, and order details using JOIN with aliases.

```

---

### SQL Session 05 Questions with Answers
