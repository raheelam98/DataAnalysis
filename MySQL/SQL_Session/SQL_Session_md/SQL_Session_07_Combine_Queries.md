...
### SQL Session 07 - Combine Queries - Questions 

```sql

## Topic: UNION, INTERSECT, EXCEPT, and Joins

### Combine Queries – Set Operations

1. Combine all members (students and organizers) using `UNION`.
2. Find the common participants between `Students_Data` and `Event_organizers` using `INTERSECT`.
3. Find students who are also organizers using `INNER JOIN`.
4. Identify students who are not organizers using `EXCEPT`.
5. Identify students who are not organizers using `LEFT JOIN` and `NULL` check.

```
--- 
### SQL Session 07 - Combine Queries - Questions with Answers
